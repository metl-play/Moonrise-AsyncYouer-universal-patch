...
public final class MoonriseCommonInternal {

    private static final Logger LOGGER = LoggerFactory.getLogger(MoonriseCommonInternal.class);
    ...
    public static final PrioritisedThreadPool.ExecutorGroup RENDER_EXECUTOR_GROUP = MoonriseCommonInternal.WORKER_POOL.createExecutorGroup(CLIENT_DIVISION, 0);
    public static final int SERVER_DIVISION = 1;
    public static final PrioritisedThreadPool.ExecutorGroup PARALLEL_GEN_GROUP = MoonriseCommonInternal.WORKER_POOL.createExecutorGroup(SERVER_DIVISION, 0);
    public static final PrioritisedThreadPool.ExecutorGroup RADIUS_AWARE_GROUP = MoonriseCommonInternal.WORKER_POOL.createExecutorGroup(SERVER_DIVISION, 0);
    public static final PrioritisedThreadPool.ExecutorGroup LOAD_GROUP         = MoonriseCommonInternal.WORKER_POOL.createExecutorGroup(SERVER_DIVISION, 0);
    ...
    public static void haltExecutors() {
        MoonriseCommonInternal.WORKER_POOL.shutdown(false);
        LOGGER.info("Awaiting termination of worker pool for up to 60s...");
        if (!MoonriseCommonInternal.WORKER_POOL.join(TimeUnit.SECONDS.toMillis(60L))) {
            LOGGER.error("Worker pool did not shut down in time!");
            MoonriseCommonInternal.WORKER_POOL.halt(false);
        }

        MoonriseCommonInternal.IO_POOL.shutdown(false);
        LOGGER.info("Awaiting termination of I/O pool for up to 60s...");
        if (!MoonriseCommonInternal.IO_POOL.join(TimeUnit.SECONDS.toMillis(60L))) {
            LOGGER.error("I/O pool did not shut down in time!");
            MoonriseCommonInternal.IO_POOL.halt(false);
        }
    }

    private MoonriseCommonInternal() {}
}
